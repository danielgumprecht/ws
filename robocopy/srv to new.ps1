...
robocopy "\\$srv\clientapps$\_Backups\$env:USERNAME\Edge" "$env:localappdata\Microsoft\Edge" /E /MIR /W:0 /V

# Copy Chrome
Get-Process -Name chrome -ErrorAction SilentlyContinue | Stop-Process -Force
robocopy "\\$srv\clientapps$\_Backups\$env:USERNAME\Chrome" "$env:localappdata\Google\Chrome" /E /MIR /W:0 /V

# Copy Firefox
Get-Process -Name firefox -ErrorAction SilentlyContinue | Stop-Process -Force
robocopy "\\$srv\clientapps$\_Backups\$env:USERNAME\Firefox" "$env:localappdata\Mozilla\Firefox" /E /MIR /W:0 /V
